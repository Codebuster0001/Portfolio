    ...
    <div className="p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">Manage and respond to portfolio inquiries.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-wrap gap-4 items-center bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input 
                ...
                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-blue-500 transition-colors"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}
              className="bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-sm text-zinc-300 focus:outline-none focus:border-blue-500"
            >
                  ...
                  className={`p-4 rounded-xl border cursor-pointer transition-all ${
                    selectedContact?.id === contact.id 
                      ...
                      : !contact.isRead
                        ...
                        : 'bg-zinc-900/30 border-zinc-800 hover:border-zinc-700 opacity-70'
                  }`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className={`font-semibold ${!contact.isRead ? 'text-white' : 'text-zinc-300'}`}>
                          {contact.name}
                        </h3>
                  ...
                  <p className="text-sm text-zinc-400 line-clamp-2 ml-7">
                    {contact.message}
                  </p>
                </div>
              ))}
          ...
          {contactsData?.totalPages > 1 && (
            <div className="flex justify-between items-center py-4">
              <button 
                ...
                className="p-2 rounded bg-zinc-900 text-white disabled:opacity-50"
              ><ChevronLeft className="w-4 h-4" /></button>
              <span className="text-sm text-zinc-400">Page {page} of {contactsData.totalPages}</span>
              <button 
        ...
        <div className="lg:col-span-1">
          {selectedContact ? (
            <div className="sticky top-6 bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Received</label>
                ...
                <div className="pt-4">
                  <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2 block">Message</label>
                ...
                <div className="pt-4 border-t border-zinc-800">
                  <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider block mb-2">Reply to Sender</label>
                  ...
                  <textarea
                    ...
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-blue-500 transition-colors mb-3 resize-none"
                  />
                  <button
                ...
                {/* Metadata */}
                <div className="pt-4 border-t border-zinc-800">
                  <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider block mb-2">Metadata</label>
                  <div className="text-xs text-zinc-500 space-y-1">
                    <p><span className="text-zinc-400">IP:</span> {selectedContact.ipAddress || 'Unknown'}</p>
                    <p><span className="text-zinc-400">Source:</span> {selectedContact.source}</p>
                    <p className="break-all"><span className="text-zinc-400">Agent:</span> {selectedContact.userAgent}</p>
                  </div>
                </div>
              </div>
            </div>
          ) : (
            <div className="h-full min-h-[300px] flex flex-col items-center justify-center text-zinc-500 bg-zinc-900/30 rounded-xl border border-zinc-800 border-dashed p-6 text-center">
